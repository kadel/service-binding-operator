...
func TestBuildServiceContexts(t *testing.T) {
	...
	falseBool := false
	nameStrategy := "{{ .service.kind | upper }}_{{ .name | upper }}"

	t.Run("empty selectors", func(t *testing.T) {
		ns := "planner"
		f := mocks.NewFake(t, ns)
		serviceCtxs, err := buildServiceContexts(
			logger, f.FakeDynClient(), ns, nil, &falseBool, false, restMapper, nameStrategy)
	...
	t.Run("declared and existing selectors", func(t *testing.T) {
		...
		serviceCtxs, err := buildServiceContexts(
			logger, f.FakeDynClient(), firstNamespace, sbr.Spec.Services, &falseBool, false, restMapper, nameStrategy)
	...
	t.Run("services in different namespace", func(t *testing.T) {
		...
		serviceCtxs, err := buildServiceContexts(
			logger, f.FakeDynClient(), sameNs, sbr.Spec.Services, &falseBool, false, restMapper, nameStrategy)
